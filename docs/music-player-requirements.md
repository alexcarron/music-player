...
4. For each successfully matched song, the app shall fetch and store Spotify **audio features**: BPM (tempo), energy, valence, danceability, acousticness, instrumentalness, key, loudness, and mode — persisted in the `track_analysis` table and queryable.
...
1. The song list shall support **arrow-key navigation** — up/down arrows move the selection highlight, Enter plays the highlighted song.
