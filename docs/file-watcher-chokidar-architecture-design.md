...
# The Stack
`chokidar` watches the root music folder recursively. It is initialized after the DB connection is open and IPC handlers are registered. It only watches `.mp3`, `.wav`, and `.flac` files — all other file types are ignored at the filter level before any processing happens.
# Event Flow
## File Added
...
3. Main pushes `library:file-added` to the renderer with the minimal record — the track appears in the UI immediately
4. Main enqueues an `EXTRACT_METADATA` job for the file
5. Job completes — main updates the track row with full metadata
6. Main pushes a second `library:file-added` event with the complete track data
7. Renderer's `ipcRenderer.on('library:file-added')` listener calls `libraryStore.upsertTrack(track)` — Zustand updates, React re-renders
...
4. Renderer calls `libraryStore.markUnlinked(trackId)` — Zustand updates, the track renders with its unlinked visual state
